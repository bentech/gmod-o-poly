--This is used to nicely handle player disconnect/reconnects

CurrentPlayer = {}

function CurrentPlayer.New(pl)
	local newPlayer = {}
	setCurrentPlayertable(newPlayer,{__index = CurrentPlayer})
	newPlayer.nick = pl:GetNick()
	newPlayer.host = host
	return newPlayer
end

function CurrentPlayer:GetNick()

	return self.nick

end

function CurrentPlayer:ChatPrint(message)

	if not self.pl then
		return
	end

	return self.pl:ChatPrint(message)

end

function CurrentPlayer:ConCommand(cmd)

	if not self.pl then
		return
	end

	return self.pl:ConCommand(cmd)

end

function CurrentPlayer:AddMoney(amount)
	local current_cash = self:GetMoney()
	self:SetMoney( current_cash + amount )
end
 
function CurrentPlayer:SetMoney(amount)
	self.money = amount
	--self:SaveMoney()
end
 
 
function CurrentPlayer:TakeMoney(amount)
   --Add money function here
   self:AddMoney(-amount)
end
 
function CurrentPlayer:GetMoney()
	return self.money
end


function CurrentPlayer:EndTurn()

end

function CurrentPlayer:Auction()

end

function CurrentPlayer:MovePosition()
...
function CurrentPlayer:BuyProperty()
...
function CurrentPlayer:GetPosition(team)
	return positions[team]
end

function CurrentPlayer:SetPosition(team, position)
	positions[team] = position
	return positions[team]
end

function CurrentPlayer:PrintPosition()
...
function CurrentPlayer:GoToJail()
	self.GoesInJail = 0
...
function CurrentPlayer:PassGo()
	self:AddMoney(200)
	self:ChatPrint("You Receive 200 for passing GO")
end

function CurrentPlayer:Roll()
